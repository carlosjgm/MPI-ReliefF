...
int main(int argc, char** argv) {
    ...
    //        //for each iteration
    //        double timer = 0;
    //        for (int i = rank; i < iternum; i += size) {
    //    
    //            if (i % (iternum / 25) == 0)
    ...
    //    
    //        //cout << "\n" << timer << "s\n";
    //        vector<float> attrWeightF(m);
    //        MPI_Reduce(&attrWeight[0], &attrWeightF[0], m, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
    //    
    //        if (rank == 0) {
    ...
    //                result[i].push_back(attrWeightF[i]);
    //            }
    //    
    //            //sort result
    //            for (int i = 0; i < m - 1; i++) {
...
void test(int argc, char** argv) {
    ...
    //
    //define node mpi data type
    //
    MPI_Datatype MPI_NODE;
    MPI_Datatype types[4] = {MPI_LB, MPI_INT, MPI_FLOAT, MPI_UB};
    int blocklength[4] = {1, 5, m + 2, 1};

    MPI_Aint displacements[4];

    mpiNode snode;
    MPI_Address(&snode, &displacements[0]);
    MPI_Address(&snode.index, &displacements[1]);
    MPI_Address(&snode.median, &displacements[2]);
    displacements[3] = sizeof (snode);

    for (int i = 2; i >= 0; i--) displacements[i] -= displacements[0];

    MPI_Type_create_struct(4, blocklength, displacements, types, &MPI_NODE);
    MPI_Type_commit(&MPI_NODE);

    //
    //test mpi node data type
    //    
    int size;
    MPI_Type_size(MPI_NODE, &size);
    assert(sizeof (mpiNode) == size);
    ...
    MPI_Bcast(&nodeListSize, 1, MPI_INT, 0, MPI_COMM_WORLD);

    mpiNode mpiNodeList[nodeListSize];

    if (rank == 0) {

        for (int i = 0; i < nodeListSize; i++) {
            node currNode = nodeList[i];
            mpiNodeList[i].index = currNode.getIndex();
            mpiNodeList[i].parent = currNode.getParent();
            mpiNodeList[i].left = currNode.getLeft();
            mpiNodeList[i].right = currNode.getRight();
            mpiNodeList[i].attrIndex = currNode.getAttrIndex();
            mpiNodeList[i].median = currNode.getMedian();
            if (currNode.isLeaf())
                for (int j = 0; j < m + 1; j++)
                    mpiNodeList[i].values[j] = currNode.getValues()[j];
        }
    }

    MPI_Bcast(mpiNodeList, nodeListSize, MPI_NODE, 0, MPI_COMM_WORLD);

    for (int i = 0; i < nodeListSize; i++) {
        node currNode = nodeList[i];
        mpiNode currMpiNode = mpiNodeList[i];
        assert(currMpiNode.index == currNode.getIndex());
        assert(currMpiNode.parent == currNode.getParent());
        assert(currMpiNode.left == currNode.getLeft());
        assert(currMpiNode.right == currNode.getRight());
        assert(currMpiNode.attrIndex == currNode.getAttrIndex());
        assert(currMpiNode.median == currNode.getMedian());
        if (currNode.isLeaf())
            for (int j = 0; j < m + 1; j++)
                assert(currMpiNode.values[j] == currNode.getValues()[j]);
    }

    nodeList.clear();
    for (int i = 0; i < nodeListSize; i++) {
        mpiNode currMpiNode = mpiNodeList[i];
        nodeList.push_back(node(currMpiNode.index, currMpiNode.parent, currMpiNode.left,
                currMpiNode.right, currMpiNode.attrIndex, currMpiNode.median, currMpiNode.values, m + 1));
    }

    kdTree mpiTree(nodeList, m, nominal);
    j = 0;
    for (int i = 0; i < nodeList.size(); i++)
        if (nodeList[i].isLeaf()) {
            vector<float> tempvalues = nodeList[i].getValues();
    ...
    neighbors = mpiTree.kNeighborSearch(observation, 2);
    assert(neighbors.size() == 2);
    for (int j = 0; j < m; j++)
        assert(neighbors[0][j] == dataset[8 * (m + 1) + j]);
    for (int j = 0; j < m; j++)
        assert(neighbors[1][j] == dataset[5 * (m + 1) + j]);
    neighbors = mpiTree.kNeighborSearch(observation, 1);
    assert(neighbors.size() == 1);
    for (int j = 0; j < m; j++)
        assert(neighbors[0][j] == dataset[8 * (m + 1) + j]);

    cout << "Rank " << rank << ": All tests passed." << endl;
